...
export function StartSessionDialog({ table, open, onOpenChange }: Props) {
  ...
  const submit = () => {
    ...
    startSession({ tableId: table.id, customerName: name.trim(), customerPhone: phone.trim() });
    ...
    <Dialog open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) reset(); }}>
      <DialogContent className="glass">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Used for WhatsApp thank-you after payment.
            </p>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} className="glow-neon">Start session</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
